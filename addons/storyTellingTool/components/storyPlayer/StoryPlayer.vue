...
export default {
  ...
  methods: {
    ...
    activateTool(toolId) {
      ...
      if (tool) {
        this.$store.commit(`Tools/${tool.component.name}/setActive`, true);
      }
    },
    ...
    loadStep() {
      ...
      for (const layer of layerList) {
        const isStepLayer = (this.currentStep.layers || []).includes(layer.id);
      ...
      .then(console.log(this.loadedContent))



      // Load HTML file for the story step - OLD, with no backend
      //fetchDataFromUrl(
      //   "./assets/" +
