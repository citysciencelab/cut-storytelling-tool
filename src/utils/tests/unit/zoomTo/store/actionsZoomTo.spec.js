...
describe("src/utils/zoomTo/store/actionsZoomTo.js", () => {
    describe("zoomToFeatures", () => {
        ...
        it("should fetch features and call respective vuex store functions if a correct (but deprecated) configuration and url parameter match for zoomToFeature is prov", async () => {
            getters.deprecatedParameters = true;
            getters.config.zoomToFeature = {
                wfsId: id,
                attribute: "flaechenid",
            ...
            expect(commit.firstCall.args[0]).to.equal("Map/addLayerToMap");
            ...
            expect(dispatch.firstCall.args[0]).to.equal("Map/zoomTo");
            expect(dispatch.firstCall.args[1] instanceof Object).to.be.true;
            expect(Object.prototype.hasOwnProperty.call(dispatch.firstCall.args[1], "geometryOrExtent")).to.be.true;
            expect(dispatch.firstCall.args[1].geometryOrExtent.length).to.equal(4);
            expect(dispatch.firstCall.args[1].geometryOrExtent.every(val => typeof val === "number")).to.be.true;
        ...
        it("should add features to the map for one working config (zoomToFeatureId) and dispatch an alert for a configuration with an invalid id if both are present", async () => {
            ...
            expect(dispatch.firstCall.args).to.eql(["Alerting/addSingleAlert", "utils.parametricURL.zoomTo", {root: true}]);
            expect(dispatch.secondCall.args.length).to.equal(3);
            expect(dispatch.secondCall.args[0]).to.equal("Map/zoomTo");
            expect(dispatch.secondCall.args[1] instanceof Object).to.be.true;
            expect(Object.prototype.hasOwnProperty.call(dispatch.secondCall.args[1], "geometryOrExtent")).to.be.true;
            expect(dispatch.secondCall.args[1].geometryOrExtent.length).to.equal(4);
            expect(dispatch.secondCall.args[1].geometryOrExtent.every(val => typeof val === "number")).to.be.true;
