...
export default function convertSexagesimalToDecimal (coord) {
    if (typeof coord === "string" && coord.length > 14) {
        const eastingSplit = coord.substr(0, 13).trim().split(" "), // 53° 32′ 24″ N
            northingSplit = coord.substr(14).trim().split(" "), // 9° 54′ 56″ E
            eastingDegree = Number(eastingSplit[0].substr(0, eastingSplit[0].length - 1)),
