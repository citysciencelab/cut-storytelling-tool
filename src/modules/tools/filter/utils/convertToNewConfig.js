...
function convertToNewConfig (oldConfig) {
    ...
    const clonedConfig = cloneObject(oldConfig?.filter),
        ...
        clonedPredefinedQueries = cloneObject(newConfig.filter?.predefinedQueries);

    if (!newConfig.filter) {
        return {};
    }

    delete newConfig.filter.predefinedQueries;
    delete newConfig.filter.isGeneric;
    if (oldConfig?.saveToUrl) {
        newConfig.filter.saveTo = oldConfig.saveToUrl ? "url" : "void";
    }
    delete newConfig.filter.saveToUrl;
    delete newConfig.filter.allowMultipleQueriesPerLayer;

    if (clonedPredefinedQueries.filter(query => query?.snippetType === "checkbox-classic").length > 0) {
        newConfig.filter.layerSelectorVisible = false;
        newConfig.filter.renderFilterInto = "tableMenu";
    }
    newConfig.filter.layers = convertPredefinedQueryToLayer(clonedPredefinedQueries, oldConfig.deactivateGfi);
    return newConfig;
}
